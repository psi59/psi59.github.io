...
MySQL 8.0 버전부터 권한을 묶어서 역할을 사용할 수 있게 됐다. 실제 내부적으로는 역할은 계정 객체와 똑같은 모습을 하고 있다. 따라서 `role_`과 같은 프리픽스를 부여해서 계정이랑 구분할 수 있도록 생성하는 것이 좋다.

역할은 `CREATE ROLE` 명령을 이용해서 정의할 수 있다. 처음 역할을 생성하면 아무 권한이 존재하지 않지만 `GRANT` 명령으로 역할을 부여하면 된다.
